...
<button type="button" className="hidden_border btn_bg" onClick={()=>
currentPage>1? 
    dispatch(setCurrentPage(currentPage-1))
:console.log('out of number of pages')}> 
 <img src={arrowLeft} alt="React Logo" style={{width:'30px',height:'30px'}}/></button>

 <button type="button" className="hidden_border btn_bg p-0" onClick={()=>
 currentPage<=Math.ceil(totalPets/postPerPage)-1?
   dispatch(setCurrentPage(currentPage+1))
   :console.log('out of number of pages')}>
     <img src={arrowRight} alt="React Logo" style={{width:'30px',height:'30px'}}/> </button>
  </div>
</> )
