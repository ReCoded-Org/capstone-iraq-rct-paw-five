...
const {currentPets,currentPage,postPerPage,filteredPets} = useSelector(state => state.pets)
...
useEffect(()=>{
  ...
  window.addEventListener('resize', handleResize)
    ...
    if(width<=569){dispatch(setPostPerPage(2))}
    if(width>582 && width<=982 ){dispatch(setPostPerPage(4))}
    if(width>982){dispatch(setPostPerPage(6))}
