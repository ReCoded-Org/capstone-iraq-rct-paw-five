...
const filter = (pets, filters) => {
  ...
  if (filters.gender) {
    if(filters.gender!=='all')filteredArray = filteredArray.filter(item => item.gender === filters.gender)
  }
  if (filters.address) {
    if(filters.address!=='all')filteredArray = filteredArray.filter(
      item => item.address === filters.address
    )
  }
  if (filters.age) {
    if(filters.age!=='all')filteredArray = filteredArray.filter(item =>
